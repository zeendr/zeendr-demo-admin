    ...
    <Container style={{ 
      ...
      <Box sx={{ padding: '3rem 0', textAlign: isSmallScreen ? 'center' : 'initial' }}>
        <Grid container spacing={4} alignItems="center">
          ...
          <Grid item xs={12} md={6}>
            ...
            <Button variant="contained" size="large" sx={{mt: 2, backgroundColor: '#EFE8DD',color: 'black', borderRadius: '10px', '&:hover': { backgroundColor: '#f3eee7' },}}>
              Empezar
            </Button>
          </Grid>
        </Grid>
      ...
      <Box sx={{ margin: '4rem 0' }}>
        ...
        <Grid container spacing={4} justifyContent="center">
          {[
            // Asegúrate de agregar el campo 'link' a cada objeto
            ...
            <Grid key={index} item xs={12} sm={6} md={4}>
              {/* Aquí envolvemos el Card con Link */}
              <Link to={item.link} style={{ textDecoration: 'none', color: 'inherit' }}>
                <Card sx={cardStyles}>
                  <CardHeader
                    avatar={<Avatar aria-label="feature" sx={{ backgroundColor: '#EFE8DD'}}>{item.icon || item.title.charAt(0)}</Avatar>}
                    title={item.title}
                  />
                  <CardContent>
      ...
      <Box sx={{ padding: '3rem 0', backgroundColor: '#fffff' }}>
